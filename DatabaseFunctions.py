...
def getRndDate():
    stDte = datetime.date(1940,1,1)
    edDte = datetime.date(2008,12,31)
...
def addRndDisease(conn, sickUUID):
    ...
    if idPres == 0:
        print(diseaseID, " not found, no condition added")
    else:
        ...
        if idPres == 0:
            ...
            print("Added condition ", diseaseID, " to uuid ", sickUUID)
        else:
    ...
    for usrRow in userDetails:
        for x in range (20, 28):
            for y in range (8, 23):
                ...
                if rndVis == 1:
                    bDT = datetime(2020, 11, x, y)
                    eDT = datetime(2020, 11, x, y+1)
...
conn = createConnection("safeTrip", "ST_Group", "safetripPass",
"psql.moostermiko.com", "5432")
